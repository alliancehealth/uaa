...
public class ResetPasswordAuthenticationEntryPointTests {
    ...
    public void test_invalid_password_match() throws Exception {
        ...
        verify(request, times(1)).setAttribute(eq("email"), eq(email));
        verify(request, times(1)).setAttribute(eq("code"), eq(code));

        verify(requestDispatcher, timeout(1)).forward(same(request), same(response));
